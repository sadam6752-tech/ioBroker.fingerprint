'use strict';

const http = require('node:http');

/**
 * Lightweight HTTP client for the FingerprintDoorbell ESP32 WebUI.
 * Uses the built-in http module (no extra dependency) and HTTP Basic Auth.
 */
class EspClient {
    /**
     * @param {object} opts options
     * @param {string} opts.ip device IP address
     * @param {number} opts.port device HTTP port
     * @param {number} opts.timeout request timeout in ms
     * @param {string} [opts.user] HTTP Basic Auth user
     * @param {string} [opts.password] HTTP Basic Auth password
     */
    constructor(opts) {
        this.ip = opts.ip;
        this.port = opts.port || 80;
        this.timeout = opts.timeout || 5000;
        this.user = opts.user || '';
        this.password = opts.password || '';
    }

    /**
     * Perform a GET request against the device.
     *
     * @param {string} path request path (e.g. '/debug')
     * @returns {Promise<{status: number, body: string}>} resolves with status code and body
     */
    _get(path) {
        return new Promise((resolve, reject) => {
            const headers = {};
            if (this.user || this.password) {
                const token = Buffer.from(`${this.user}:${this.password}`).toString('base64');
                headers.Authorization = `Basic ${token}`;
            }

            let settled = false;
            const done = (err, value) => {
                if (settled) {
                    return;
                }
                settled = true;
                if (err) {
                    reject(err);
                } else {
                    resolve(value);
                }
            };

            const req = http.request(
                {
                    host: this.ip,
                    port: this.port,
                    path,
                    method: 'GET',
                    headers,
                    timeout: this.timeout,
                },
                res => {
                    let body = '';
                    res.setEncoding('utf8');
                    res.on('data', chunk => (body += chunk));
                    res.on('end', () => done(null, { status: res.statusCode, body }));
                },
            );

            // The request's own `timeout` option guards against a hanging socket:
            // on timeout we destroy the request, which surfaces as an 'error'.
            req.on('error', err => done(err));
            req.on('timeout', () => {
                req.destroy(new Error(`Request to ${this.ip}:${this.port}${path} timed out`));
            });
            req.end();
        });
    }

    /**
     * Perform a POST request with a text/JSON body.
     *
     * @param {string} path request path (e.g. '/restore')
     * @param {string} body request body
     * @param {number} [timeout] optional timeout override (ms)
     * @returns {Promise<{status: number, body: string}>} resolves with status code and body
     */
    _post(path, body, timeout) {
        const tmo = timeout || this.timeout;
        return new Promise((resolve, reject) => {
            const payload = Buffer.from(body || '', 'utf8');
            const headers = {
                'Content-Type': 'application/json',
                'Content-Length': payload.length,
            };
            if (this.user || this.password) {
                const token = Buffer.from(`${this.user}:${this.password}`).toString('base64');
                headers.Authorization = `Basic ${token}`;
            }

            let settled = false;
            const done = (err, value) => {
                if (settled) {
                    return;
                }
                settled = true;
                if (err) {
                    reject(err);
                } else {
                    resolve(value);
                }
            };

            const req = http.request(
                { host: this.ip, port: this.port, path, method: 'POST', headers, timeout: tmo },
                res => {
                    let resBody = '';
                    res.setEncoding('utf8');
                    res.on('data', chunk => (resBody += chunk));
                    res.on('end', () => done(null, { status: res.statusCode, body: resBody }));
                },
            );

            // The request's own `timeout` option guards against a hanging socket.
            req.on('error', err => done(err));
            req.on('timeout', () => {
                req.destroy(new Error(`POST to ${this.ip}:${this.port}${path} timed out`));
            });
            req.write(payload);
            req.end();
        });
    }

    /**
     * Check whether the device is reachable via the /debug endpoint (no auth required).
     *
     * @returns {Promise<{reachable: boolean, info: object}>} reachability plus parsed device info
     */
    async ping() {
        try {
            const res = await this._get('/debug');
            if (res.status !== 200) {
                return { reachable: false, info: {} };
            }
            return { reachable: true, info: this._parseDebug(res.body) };
        } catch {
            return { reachable: false, info: {} };
        }
    }

    /**
     * Parse the plain-text /debug output into a key/value object.
     *
     * @param {string} text raw /debug body
     * @returns {object} parsed key/value pairs
     */
    _parseDebug(text) {
        const info = {};
        for (const line of String(text).split('\n')) {
            const idx = line.indexOf(':');
            if (idx > 0) {
                const key = line.slice(0, idx).trim();
                const val = line.slice(idx + 1).trim();
                if (key) {
                    info[key] = val;
                }
            }
        }
        return info;
    }

    /**
     * Trigger a device reboot via the /reboot endpoint.
     *
     * @returns {Promise<boolean>} true on success
     */
    async reboot() {
        const res = await this._get('/reboot');
        // /reboot answers with a redirect (302) or 200 before rebooting
        return res.status >= 200 && res.status < 400;
    }

    /**
     * Enable/disable "ignore touch ring" via the /set-touch-ring endpoint.
     * Requires FingerprintDoorbell firmware >= v0.9.1.
     *
     * @param {boolean} enabled desired state
     * @returns {Promise<boolean>} true on success
     */
    async setIgnoreTouchRing(enabled) {
        const res = await this._get(`/set-touch-ring?state=${enabled ? 'on' : 'off'}`);
        return res.status >= 200 && res.status < 400;
    }

    /**
     * Query the device status via the JSON /api/status endpoint (firmware >= v0.9.1).
     *
     * @returns {Promise<{reachable: boolean, status: object}>} reachability plus parsed status object
     */
    async getStatus() {
        try {
            const res = await this._get('/api/status');
            if (res.status !== 200) {
                return { reachable: false, status: {} };
            }
            let status = {};
            try {
                status = JSON.parse(res.body);
            } catch {
                return { reachable: false, status: {} };
            }
            return { reachable: true, status };
        } catch {
            return { reachable: false, status: {} };
        }
    }

    /**
     * Register this adapter as the event target on the device (server mode).
     * Requires firmware >= v0.9.1.
     *
     * @param {string} host adapter host/IP the device should send events to
     * @param {number} port webhook port
     * @param {string} token shared secret
     * @returns {Promise<boolean>} true on success
     */
    async registerServer(host, port, token) {
        const q = `host=${encodeURIComponent(host)}&port=${encodeURIComponent(port)}&token=${encodeURIComponent(token)}`;
        const res = await this._get(`/api/register-server?${q}`);
        return res.status >= 200 && res.status < 400;
    }

    /**
     * Enable/disable server mode on the device.
     *
     * @param {boolean} enabled desired state
     * @returns {Promise<boolean>} true on success
     */
    async setServerMode(enabled) {
        const res = await this._get(`/api/server-mode?state=${enabled ? 'on' : 'off'}`);
        return res.status >= 200 && res.status < 400;
    }

    /**
     * Fetch the list of enrolled fingerprints via /api/fingerprints (firmware >= v0.9.1).
     *
     * @returns {Promise<Array<{id: number, name: string}>>} list of fingerprints (empty on failure)
     */
    async getFingerprints() {
        try {
            const res = await this._get('/api/fingerprints');
            if (res.status !== 200) {
                return [];
            }
            const list = JSON.parse(res.body);
            return Array.isArray(list) ? list : [];
        } catch {
            return [];
        }
    }

    /**
     * Rename a finger via the /add-name endpoint (firmware v0.9+).
     *
     * @param {number} id fingerprint id (1–200)
     * @param {string} name new name
     * @returns {Promise<boolean>} true on success
     */
    async renameFinger(id, name) {
        const q = `id=${encodeURIComponent(id)}&name=${encodeURIComponent(name)}`;
        const res = await this._get(`/add-name?${q}`);
        return res.status >= 200 && res.status < 400;
    }

    /**
     * Start enrollment of a new fingerprint on the device (firmware >= v0.9.4).
     * The device switches into enroll mode and returns immediately; the user
     * must place the finger 5 times. Progress is reported via /api/status
     * (enrollState / enrollStep / enrollMessage).
     *
     * @param {number} id fingerprint slot id (1–200)
     * @param {string} name name for the new finger
     * @returns {Promise<{ok: boolean, status: number, body: string}>} start result
     */
    async startEnroll(id, name) {
        const q = `id=${encodeURIComponent(id)}&name=${encodeURIComponent(name || '')}`;
        const res = await this._post(`/api/enroll?${q}`, '', this.timeout);
        return { ok: res.status >= 200 && res.status < 300, status: res.status, body: res.body };
    }

    /**
     * Control the sensor LED ring (firmware >= v0.9.4).
     *
     * @param {number} mode 0=off, 1=on, 2=breathing, 3=flashing
     * @param {number} color 1=red, 2=blue, 3=purple, 4=green, 5=yellow, 6=cyan, 7=white
     * @param {number} [speed] 0..255 (breathing/flashing speed)
     * @param {number} [cycles] 0..255 (0 = infinite)
     * @returns {Promise<boolean>} true on success
     */
    async setLed(mode, color, speed = 0, cycles = 0) {
        const q = `mode=${encodeURIComponent(mode)}&color=${encodeURIComponent(color)}&speed=${encodeURIComponent(speed)}&cycles=${encodeURIComponent(cycles)}`;
        const res = await this._get(`/api/led?${q}`);
        return res.status >= 200 && res.status < 400;
    }

    /**
     * Download the fingerprint backup (JSON array of {id,name,size,template}) via /backup.
     * This can take a while as templates are read from the sensor, so a longer timeout is used.
     *
     * @returns {Promise<{ok: boolean, json: string, count: number}>} backup result
     */
    async getBackup() {
        // Reading templates from the sensor is slow → allow up to 30s
        const res = await this._get2('/backup', 30000);
        if (res.status !== 200) {
            return { ok: false, json: '', count: 0 };
        }
        let count = 0;
        try {
            const arr = JSON.parse(res.body);
            count = Array.isArray(arr) ? arr.length : 0;
        } catch {
            return { ok: false, json: '', count: 0 };
        }
        return { ok: true, json: res.body, count };
    }

    /**
     * Restore fingerprints by POSTing the backup JSON to /restore.
     *
     * @param {string} json backup JSON (array of {id,name,size,template})
     * @returns {Promise<boolean>} true on success
     */
    async restore(json) {
        // Writing templates to the sensor is slow → allow up to 60s
        const res = await this._post('/restore', json, 60000);
        return res.status >= 200 && res.status < 400;
    }

    /**
     * GET with a custom timeout (wrapper around the internal request with overridden timeout).
     *
     * @param {string} path request path
     * @param {number} timeout timeout in ms
     * @returns {Promise<{status: number, body: string}>} response
     */
    _get2(path, timeout) {
        const original = this.timeout;
        this.timeout = timeout;
        const p = this._get(path);
        // restore default timeout for subsequent calls once this request settles
        p.finally(() => {
            this.timeout = original;
        });
        return p;
    }
}

module.exports = EspClient;
